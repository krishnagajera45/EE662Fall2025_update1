...
CONNECTIVITY_CHECK_INTERVAL = 100.0
CONNECTIVITY_CHECK_START_DELAY = 500.0
ALL_NODES = []
CLUSTER_HEADS = []
ROLE_COUNTS = Counter()
...
# Convert address to hashable tuple for dictionary keys
def addr_key(addr):
    if addr is None:
        return None
    return (getattr(addr, 'net_addr', None), getattr(addr, 'node_addr', None))

# Format network address as string [net_addr,node_addr]
def format_addr(addr):
    if addr is None:
        return "None"
    return f"[{addr.net_addr},{addr.node_addr}]"

# Safely compare two addresses, handling None values
def addr_equals(a, b):
...
# Generate and return next unique packet ID
def next_packet_id():
    global DATA_PACKET_COUNTER
    DATA_PACKET_COUNTER += 1
    return DATA_PACKET_COUNTER

# Log node power level to CSV file over time
...
# Copy simulation output files to results directory
def copy_simulation_outputs():
    try:
        ...
        battery_voltage = getattr(config, 'BATTERY_VOLTAGE', 3.0)
        initial_energy = battery_voltage * battery_capacity * 3600
        metadata = {
            ...
            "data_packet_interval": getattr(config, 'DATA_PACKET_INTERVAL', 10),
...
# Log packet routing decision to CSV file
...
# Log packet delivery event with timing information
...
# Record complete packet path through network
...
# Log node registration time and join delay to CSV
...
# Calculate and log average packet delay statistics
...
# Calculate and log average node join time statistics
...
# Log when a node becomes orphaned (loses parent)
...
# Log when a node changes its role in the network
...
# Log node recovery event after failure
...
# Calculate and log network recovery statistics
...
# Log packet loss statistics to console
...
# Get transmission current in mA for given TX power level
...
# Calculate energy consumed for packet transmission
...
# Calculate energy consumed for packet reception
...
# Calculate time required to transmit packet
...
# Calculate time required to receive packet
...
# Calculate processing time based on packet type
...
# Get transmission power level for specific cluster
...
# Set transmission power level for specific cluster
...
class SensorNode(wsn.Node):
    ...
    # Start node execution with arrival timer
    def run(self):
        self.set_timer('TIMER_ARRIVAL', self.arrival)

    # Log debug information to file for runtime analysis
    def debug_log(self, enabled, message):
        if enabled:
            self.log(message)
            write_log(self, message)

    # Change node role and update visual representation
    ...
    # Convert node to router role for cluster bridging
    ...
    # Find cluster member farthest from cluster head
    ...
    # Simulate node failure and handle orphaned children
    def fail_node(self):
    ...
    # Recover failed node and rejoin network
    ...
    # Handle node becoming orphaned when parent fails
    ...
    # Reset node to unregistered state and restart discovery
    ...
    # Update neighbor table with received heartbeat information
    ...
    # Process shared neighbor information from other nodes
    ...
    # Select best parent candidate and send join request
    ...
    # Share neighbor table information with neighbors
    ...
    # Shutdown node due to low energy
    ...
    # Update transmission power for cluster members
    ...
    # Route packet using mesh or tree routing
    ...
    # Request new network address from root
    ...
    # Reply with new network address to requester
    ...
    # Handle received packet based on node role
    ...
    # Handle timer events for various node operations
...
# Write pairwise node distances to CSV file
...
# Write node distance matrix to CSV file
...
# Write distances between cluster heads to CSV
...
# Write neighbor distances to CSV file
...
def generate_power_analysis_csvs():
    ...
    try:
        ...
        node_data.sort(key=lambda x: (x[0], x[1]))
